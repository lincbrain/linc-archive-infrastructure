...
data "aws_caller_identity" "sponsored_account_us_east_2" {
  provider = aws
}

# data "aws_caller_identity" "current" {}

resource "aws_s3_bucket" "lincset_bucket_us_east_2" {
  bucket = var.bucket_name
...
resource "aws_s3_bucket_server_side_encryption_configuration" "lincset_bucket_us_east_2" {
  bucket = aws_s3_bucket.lincset_bucket.id
...
resource "aws_s3_bucket_ownership_controls" "lincset_bucket_us_east_2" {
...
resource "aws_iam_user_policy" "lincset_bucket_owner_us_east_2" {
  // The Heroku IAM user will always be in the project account
  provider = aws.project
  ...
  policy = data.aws_iam_policy_document.lincset_bucket_owner_us_east_2.json
}

data "aws_iam_policy_document" "lincset_bucket_owner_us_east_2" {
  version = "2008-10-17"

  statement {
    resources = [
      "${aws_s3_bucket.lincset_bucket_us_east_2.arn}",
      "${aws_s3_bucket.lincset_bucket_us_east_2.arn}/*",
    ]
...
resource "aws_s3_bucket_policy" "lincset_bucket_policy_us_east_2" {
  provider = aws

  bucket = aws_s3_bucket.lincset_bucket_us_east_2.id
  policy = data.aws_iam_policy_document.lincset_bucket_policy_us_east_2.json
}

data "aws_iam_policy_document" "lincset_bucket_policy_us_east_2" {
  version = "2008-10-17"
...
# S3 lifecycle policy that permanently deletes objects with delete markers
# after 30 days.
resource "aws_s3_bucket_lifecycle_configuration" "expire_deleted_objects_us_east_2" {
  # Must have bucket versioning enabled first
  depends_on = [aws_s3_bucket_versioning.lincset_bucket_us_east_2]
